...
define([
  ...
  function _drawVeilOfSilence(map, regions) {
    ...
    var mPoly = L.polygon(bounds, { color: "#ff7800", weight: 2, smoothFactor: 2.0, fillColor: "#000000", fillOpacity: 0.15, clickable: false
    });
  ...
  function _geoCode(searchString, writeResultTo) {
    ...
    ////google V2
    //var geocoder = 'http://maps.google.com/maps/geo?q=';
    //var query = searchString + '+2500+Baden+AT';
    //var params = '&output=json&key=ABQIAAAAoFPgMrKsNpb99PGcVRtGlBT9043OX6Cq_sWR_8v-iDPyNVamURQs6Vz-dNAR8Znb-pArwqo2PpruvA';

    ////coudmade (alpha)
    //var geocoder = proxy + 'http://geocoding.cloudmade.com/8ee2a50541944fb9bcedded5165f09d9/geocoding/v2/find.js?query='
    //var query = searchString + '+Baden+AT';
    //var params = '';
  ...
  function _scrollMap(marker) {
    if(!marker) return;

    logger.log('--- marker', 'map', marker);
    logger.log('--- bounds', 'map', location.map.getPixelBounds()); //get map bounds (pixel)
    //map.panBy([x, y])
