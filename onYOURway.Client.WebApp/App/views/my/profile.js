define([
  ...
  var vm = function () {
    ...
    self.changePassword = ko.computed(function () {
      ...
      return new ChangePasswordViewModel(app, self, self.userName(), auth);
    });
    ...
    self.setPassword = ko.computed(function () {
      ...
      return new SetPasswordViewModel(app, self, auth);
    });
    ...
    self.load = function () { // Load user management data
      if (!startedLoad) {
        ...
        auth.getManageInfo(auth.returnUrl, true /* generateState */)
            .done(function (data) {
              if (typeof (data.localLoginProvider) !== "undefined" &&
                ...
                for (var i = 0; i < data.logins.length; i++) {
                  self.logins.push(new RemoveLoginViewModel(data.logins[i], self, auth));
                }
  ...
  function ChangePasswordViewModel(app, parent, name, auth) {
    var self = this;
  ...
  function RemoveLoginViewModel(data, parent, auth) {
    // Private state
    var self = this,
  ...
  function SetPasswordViewModel(app, parent, auth) {
    var self = this;
