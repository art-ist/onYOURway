  ...
  'providers/geocode-nominatim'
	...
	var vm = function () {
        ...
        */
		...
		var supportAutogeneratedKeyForHasTagsEntity = function (TagId) {
		    // this method must not be executed before a HasTag entity had been created!
		    // http://stackoverflow.com/questions/13796855/autogenerated-keys-for-entities-with-multipart-keys

		    var entityType = self.manager.metadataStore.getEntityType("HasTag");
		...
		self.toggleTag = function (tag) {
		    if (self.tags.indexOf(tag) >= 0) {
		        ...
		        self.tags.push(tag);
		        self.lastTag = self.manager.createEntity('HasTag', {
		            Location: self.entity,
		            TagId: tag.Id
		        });
		        supportAutogeneratedKeyForHasTagsEntity(tag.Id);
	...
	var my_fixup = function (tempValue, realValue, TagId) {
	    var ix = this._indexMap[realValue + ":::" + TagId]; //Changed line
	    if (ix === undefined) {
